...
app.get("/rsvp_db", async (req, res) => {
  try {
    // Fetch confirmed and declined RSVPs
    const confirmedResult = await db.query(
      `SELECT * FROM public.rsvps WHERE status = 'confirmed'`
    );
    const declinedResult = await db.query(
      `SELECT * FROM public.rsvps WHERE status = 'declined'`
    );
